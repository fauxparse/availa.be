...
RSpec.describe Event, :type => :model do
  ...
  let(:user) { FactoryGirl.create :user, memberships: [User::Membership.new(group: group)] }
  ...
  context "for Saturdays in 2015" do
    ...
    describe "#in_range" do
      it "matches a containing range" do
        expect(Event.in_range(Time.zone.local(2014, 1, 1), Time.zone.local(2016, 12, 31))).to include(event)
      end

      it "matches a contained range" do
        expect(Event.in_range(Time.zone.local(2015, 7, 1), Time.zone.local(2015, 7, 31))).to include(event)
      end

      it "matches a start-overlapping range" do
        expect(Event.in_range(Time.zone.local(2014, 1, 1), Time.zone.local(2015, 7, 31))).to include(event)
      end

      it "matches an end-overlapping range" do
        expect(Event.in_range(Time.zone.local(2015, 7, 1), Time.zone.local(2016, 12, 31))).to include(event)
      end

      it "does not match an earlier range" do
        expect(Event.in_range(Time.zone.local(2014, 1, 1), Time.zone.local(2014, 12, 31))).not_to include(event)
      end

      it "does not match a later range" do
        expect(Event.in_range(Time.zone.local(2016, 1, 1), Time.zone.local(2016, 12, 31))).not_to include(event)
      end
    end
